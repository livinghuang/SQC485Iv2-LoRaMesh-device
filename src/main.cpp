...
void setup()
{
    ...
    // BLE transient: run the whole boot at a low CPU clock and keep the boot LED off,
    // then restore full clock + LED once the radio is up (see near initLoRa below).
...
#ifdef SQC485IV2
    // Radio is up — the biggest RF transient is past. Restore full clock and only now
    // light the boot LED (kept off during bring-up to save headroom). See setup() top.
