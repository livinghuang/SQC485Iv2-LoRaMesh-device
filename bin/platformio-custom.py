...
# Factory channel injection (e.g. NAFCO deployment): the channel PSK is supplied
# at build time via environment variables and is NEVER stored in this (public) repo.
# Set NAFCO_CH1_NAME + NAFCO_CH1_PSK_HEX (hex string, no separators) to bake a
# secondary channel into the image. If unset, this is a no-op (plain build).
...
import os as _os
_ch1_name = _os.environ.get("NAFCO_CH1_NAME")
_ch1_psk_hex = _os.environ.get("NAFCO_CH1_PSK_HEX")
if _ch1_name and _ch1_psk_hex:
    ...
    _hdr = _os.path.join(_hdr_dir, "nafco_factory_prefs.h")
